...
def main():
    ...
    parser.add_argument('--output-format', type=str, default='hapslegend',
    ...
    assert args.output_format in {'hapslegend', 'vcf'}
    ...
    for i, split_data_dict in enumerate(split_data_dict_list, start=1):
        ...
        if args.output_format == 'vcf':
            command += ' --output-format vcf'
            output_file = '{:s}_{:d}.vcf.gz'.format(args.output_prefix, i)
        elif args.output_format == 'hapslegend':
            command += ' --output-format haps'
            output_file = '{:s}_{:d}.haps.gz'.format(args.output_prefix, i)
        command += ' --output-file ' + output_file
        system(command)
    time4 = time.time()
    print('Elapsed time for inference: {:f} [s]'.format(time4 - time3))

    if args.output_format == 'hapslegend':
        command = args.python3_bin
        command += ' ' + os.path.join(
            SCRIPT_DIR, 'merge_hapslegend_and_haps.py')
        command += ' --hap ' + args.hap_file
        command += ' --legend ' + args.legend_file
        command += ' --haps-prefix ' + args.output_prefix
        command += ' --start-index 1'
        command += ' --end-index {:d}'.format(len(split_data_dict_list))
        command += ' --output-prefix ' + args.output_prefix
        system(command)
